...
def keras_version():
    ...
    """
    tokens = keras.__version__.split('.')
    tokens = [''.join([c for c in t if c.isdigit()]) for t in tokens]
...
def keras_version_ok():
    ...
    """
    return ((keras_version() >= minimum_keras_version) | ('tf' in keras.__version__))
...
def assert_keras_version():
    ...
    assert ((keras_version() >= minimum_keras_version) | ('tf' in keras.__version__)), 'You are using keras version {}. The minimum required version is {}.'.format(detected, required)
