...
def create_models(backbone_retinanet, num_classes, weights, multi_gpu=0,
    ...
    if multi_gpu > 1:
        ...
        # mk @ SAIL:
        # make the training model to train on (multi_gpu-1) GPUs. One more - for evaluation
    ...
    # with tf.device(available_GPUs[-1]):
    prediction_model = retinanet_bbox(model=model, anchor_params=anchor_params)

    # compile model
    training_model.compile(loss={'regression'    : losses.smooth_l1(),
                                 'classification': losses.focal(gamma=3.5)},
                           loss_weights = {'regression'    : 0.8,
                                           'classification': 1.2},
...
def create_callbacks(model, training_model, prediction_model, validation_generator, args, eval_batch_size=1):
    ...
    if args.evaluation and validation_generator:
        evaluation = Evaluate(validation_generator, tensorboard=tensorboard_callback, weighted_average=args.weighted_average, eval_batch_size=eval_batch_size)
